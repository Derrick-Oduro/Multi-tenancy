<aside class="w-1/5 bg-white min-h-screen p-4">
        <nav class="space-y-4">
            ...
            <a href="{{ route('users.index') }}"class="{{ request()->is('users') ? 'bg-gray-200' : 'bg-white' }} block px-4 py-2 rounded hover:bg-gray-300">Users</a>
            @endrole
            @role('super-admin')
            <a href="{{ route('permissions.index') }}"class="{{ request()->is('permissions') ? 'bg-gray-200' : 'bg-white' }} block px-4 py-2 rounded hover:bg-gray-300">Permissions</a>
            <a href="{{ route('tenants.index') }}"class="{{ request()->is('tenants') ? 'bg-gray-200' : 'bg-white' }} block px-4 py-2 rounded hover:bg-gray-300">Tenants</a>
            @endrole
